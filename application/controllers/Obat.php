...
class Obat extends CI_Controller{
    ...
    public function index(){
        $data['pageContent'] = $this->load->view('obat/obat', '', TRUE);
        $data['pageTitle'] = "Obat-obatan";
    ...
    */
    ...
    public function obatlilt(){
        ...
        $data['allItems'] = $this->obatdb->getAll($orderBy, $orderFormat, $start, $limit);
        $data['range'] = $totalItems > 0 ? "Showing " . ($start+1) . "-" . ($start + count($data['allItems'])) . " of " . $totalItems : "";
        $data['links'] = $this->pagination->create_links();//page links
        $data['sn'] = $start+1;
        $data['cum_total'] = $this->obatdb->getItemsCumTotal();
    ...
    */
    ...
    public function add(){
        ...
        $this->form_validation->set_rules('itemName', 'Item name', ['required', 'trim', 'max_length[80]', 'is_unique[obat.name]'],
                ['required'=>"required"]);
        $this->form_validation->set_rules('itemKategori', 'kategori', ['trim', 'max_length[90]','is_unique[obat.code]']);
        $this->form_validation->set_rules('itemQuantity', 'Item quantity', ['required', 'trim', 'numeric'], ['required'=>"required"]);
        $this->form_validation->set_rules('itemMinimal', 'Item minimal', ['trim', 'numeric'], ['required'=>"required"]);
        $this->form_validation->set_rules('itemPrice', 'Item Price', ['required', 'trim', 'numeric'], ['required'=>"required"]);
        $this->form_validation->set_rules('itemCode', 'Item Code', ['required', 'trim', 'max_length[20]', 'is_unique[obat.code]'], 
        ...
        if($this->form_validation->run() !== FALSE){
             ...
             */
            $insertedId = $this->obatdb->add(set_value('itemName'), set_value('itemQuantity'), set_value('itemPrice'), set_value('itemKategori'), set_value('itemDescription'), set_value('itemCode'), set_value('itemMinimal'));
    ...
    public function updatestock(){
        ...
        if($this->form_validation->run() !== FALSE){
            ...
            $updated = $updateType === "deficit" 
                    ? 
                $this->obatdb->deficit($itemId, $qty, $desc) 
                    : 
                $this->obatdb->newstock($itemId, $qty, $desc);
            ...
            $eventDesc = "<p>{$qty} quantities of {$this->genmod->gettablecol('obat', 'name', 'id', $itemId)} was {$action} stock</p>
                Reason: <p>{$desc}</p>";
    ...
    */
    ...
    public function delete(){
        ...
        if($item_id){
            $this->db->where('id', $item_id)->delete('obat');
