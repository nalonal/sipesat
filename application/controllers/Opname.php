...
class Opname extends CI_Controller{
    ...
    public function index(){
        ...
        $data['pageContent'] = $this->load->view('opname/opname', $transData, TRUE);
        $data['pageTitle'] = "Opname";
                /*buat dashboard*/
        $temp = $this->kurang();
        $data['item'] = $temp['item'];
        $data['obat'] = $temp['obat'];
    ...
    */
    ...
    private function insertTrToDb($arrOfItemsDetails, $cust_name){
        ...
        foreach($arrOfItemsDetails as $get){
             ...
             */
            $transId = $this->transaction->add($itemName, $itemCode, "", $qtySold, $unitPrice, $totalPrice, $cust_name, $ref);
    ...
    */
    ...
    public function report($tanggal, $jenis, $nomor){        
        //get all transactions from db ranging from $from_date to $to_date
        $data['jenis'] = $jenis;
        $data['tanggal'] = $tanggal;
        $data['nomor'] = $nomor;

        $data['allTransactions'] = $this->transaction->getData($jenis);

        $this->load->view('opname/opnameReport', $data);
    }
}
